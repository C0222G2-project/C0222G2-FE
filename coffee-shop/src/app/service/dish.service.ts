...
export class DishService {
  ...
  deleteDishById(id: number): Observable<Dish> {
    // @ts-ignore
    return this.httpClient.patch<Dish>(this.URL_DISH + "/delete/" + id, {headers: new HttpHeaders({'authorization': this.header})}).pipe()
  }
  ...
  getDishType(): Observable<Dish[]> {
    return this.httpClient.get<Dish[]>(this.URL_DISH + "/getDishTypePage" , {headers: new HttpHeaders({'authorization': this.header})}).pipe()
  }
